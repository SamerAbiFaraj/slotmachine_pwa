    ...
    return (
        <header className="h-[50px] md:h-[70px] landscape:h-[50px] px-4 md:px-6 flex items-center justify-between z-50 shrink-0 border-b border-white/5 bg-neo-bg/80 backdrop-blur-md">
            ...
            <div className="flex items-center gap-2 md:gap-4">
                <div className="w-6 h-6 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-gradient-to-br from-neo-gold to-neo-goldDim flex items-center justify-center shadow-[0_0_20px_rgba(226,182,89,0.3)]">
                    <span className="font-display font-bold text-black text-sm md:text-2xl">R</span>
                </div>
            ...
            <div className="absolute left-1/2 -translate-x-1/2 hidden sm:flex items-center gap-4">
                {phase === GamePhase.WAITING_FOR_BETS ? (
                    <div className="glass-panel px-3 md:px-6 py-1 md:py-2 rounded-full flex items-center gap-2 md:gap-4 border-neo-accent/30">
                        <Timer timeLeft={timeLeft} totalTime={GAME_CONFIG.BETTING_DURATION_SEC} />
                        <span className="text-[10px] md:text-xs text-neo-accent font-bold uppercase tracking-widest whitespace-nowrap landscape:hidden xs:inline">Place Your Bets</span>
                    </div>
                ) : (
                    <div className="glass-panel px-4 md:px-8 py-1 md:py-2 rounded-full border-red-500/30 flex items-center gap-2 md:gap-3">
                        ...
                        <span className="text-red-400 text-[10px] md:text-xs font-bold uppercase tracking-widest whitespace-nowrap">
                            {phase === GamePhase.SPINNING ? (
                                <><span className="landscape:hidden xs:inline">No More Bets</span><span className="hidden landscape:inline xs:hidden">Locked</span></>
                            ) : 'Resolving'}
                        </span>
                    </div>
                )}
